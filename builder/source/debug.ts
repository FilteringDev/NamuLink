...
Watcher.on('all', async (WatcherEvent, WatcherPath) => {
  ...
  BuildCooldownTimer = setTimeout(async () => {
    ...
    await Build({ Version: `0.0.${Version}`, Minify: false, UseCache: true, BuildType: 'development', SubscriptionUrl: `http://localhost:${RandomPort}/NamuLink.dev.user.js` })
    Version++
...
RunDebugServer(RandomPort, ['NamuLink.dev.user.js'], ShouldPreventHTTPResponse)
console.log(`Debug HTTP server running on http://localhost:${RandomPort}/NamuLink.dev.user.js`)